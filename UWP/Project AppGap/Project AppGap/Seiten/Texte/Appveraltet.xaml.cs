...
namespace Project_AppGap.Seiten.Texte
{
    ...
    /// </summary>
    public sealed partial class Appveraltet : Page
    {        //Random Texte
        ...
        public Appveraltet()
        {
            ...
            string n_s1 = loader.GetString("appveraltet_vorlage_1");
            n_s1 = n_s1.Replace("@%", "");
            V1.Text = n_s1;

            string n_s2 = loader.GetString("appveraltet_vorlage_2");
            n_s2 = n_s2.Replace("@%", "");
            V2.Text = n_s2;

            string n_s3 = loader.GetString("appveraltet_vorlage_3");
            n_s3 = n_s3.Replace("@%", "");
            V3.Text = n_s3;
        ...
        private async void MailButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (Piv.SelectedIndex == 0)
            {
                string n_s = loader.GetString("appveraltet_vorlage_1");
                n_s = n_s.Replace("@%", "%0d%0A%0d%0A");
                var success = await Launcher.LaunchUriAsync(new Uri("mailto:?Subject=" + "Ihre App ist Veraltet" + "&Body=" + n_s, UriKind.Absolute));
            }

            if (Piv.SelectedIndex == 1)
            {
                string n_s = loader.GetString("appveraltet_vorlage_2");
                n_s = n_s.Replace("@%", "%0d%0A%0d%0A");
                var success = await Launcher.LaunchUriAsync(new Uri("mailto:?Subject=" + "Ihre App ist Veraltet" + "&Body=" + n_s, UriKind.Absolute));
            }
            ...
            if (Piv.SelectedIndex == 2)
            {
                string n_s = loader.GetString("appveraltet_vorlage_3");
                n_s = n_s.Replace("@%", "%0d%0A%0d%0A");
                var success = await Launcher.LaunchUriAsync(new Uri("mailto:?Subject=" + "Ihre App ist Veraltet" + "&Body=" + n_s, UriKind.Absolute));
            }
